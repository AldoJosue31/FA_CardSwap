...
export default function Match({ difficulty, onReturnToMenu, onNextLevel }: { difficulty?: string, onReturnToMenu?: () => void, onNextLevel?: (diff: string) => void }) {
  ...
  useEffect(() => {
    if (status === 'gameover' && playerScore > botScore && difficulty) {
  ...
  }, [status, playerScore, botScore, difficulty]);
  ...
  const hasNextLevel = currentLevelIndex < 3;
  ...
  return (
    <div className="h-screen w-full bg-[#143d22] flex flex-col justify-between font-sans relative overflow-hidden text-white selection:bg-cyan-500/30">
      ...
      {/* CEMENTERIOS / DESCARTES (IZQUIERDA) */}
      ...
      <header className="relative z-10 flex justify-center pt-4 shrink-0 pointer-events-none">
        <div className="bg-black/60 backdrop-blur-lg border border-white/10 px-6 md:px-10 py-2 md:py-3 rounded-full flex gap-8 md:gap-16 items-center shadow-[0_15px_35px_rgba(0,0,0,0.5)] pointer-events-auto">
          ...
          <div className="text-[10px] md:text-xs font-medium tracking-[0.2em] text-slate-300 uppercase">{message}</div>
          <div className="text-xl md:text-3xl font-black text-red-500 tracking-tighter">BOT: {botScore}</div>
        </div>
      </header>
