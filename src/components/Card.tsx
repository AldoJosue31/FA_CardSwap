...
export default function Card({ card, onClick, disabled, draggable, onDragStart }: CardProps) {
  ...
  return (
    <motion.div
      ...
      whileHover={!disabled && !isBot ? { scale: 1.05, translateY: -8 } : {}}
      onClick={!disabled ? onClick : undefined}
      className={`relative w-32 h-48 sm:w-40 sm:h-56 rounded-2xl flex flex-col justify-between p-2.5 select-none overflow-hidden transition-all duration-300
        ${isBot 
          ? 'bg-gradient-to-br from-red-950 via-black to-black border border-red-800/40 shadow-[0_8px_25px_rgba(0,0,0,0.5)]' 
          : 'bg-gradient-to-br from-slate-800 via-slate-900 to-black border border-slate-700/50 shadow-[0_8px_25px_rgba(0,0,0,0.4)] cursor-pointer'} 
        ${disabled ? 'opacity-60 cursor-not-allowed' : 'ring-1 ring-white/10'}`}
    >
      {/* Reflejo de luz sutil superior */}
      ...
      {/* Contenedor de Foto simplificado */}
      <div className={`relative z-10 flex-1 my-2.5 rounded-lg flex items-center justify-center overflow-hidden ${isBot ? 'bg-red-900/20' : 'bg-slate-800/30'}`}>
        {/* Silueta de degradado interna para dar profundidad */}
